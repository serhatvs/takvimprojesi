...
- `apps/api/src/events`: Event lifecycle servisi, `POST /events` taslak etkinlik olusturma, `POST /events/:eventId/submit` onaya gonderme ve Basin Yayin review karar endpointleri controller/service/dto siniri.
...
- `packages/contracts`: Paylasilan API tipleri, auth principal response tipleri, draft/event response ve review request tipleri, roller, etkinlik durumlari ve lifecycle contractlari.
