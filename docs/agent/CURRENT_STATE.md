...
- QR tabanlı güvenli katılım altyapısı eklendi: Dedicated `AttendanceModule`, `AttendanceController`, `AttendanceService` ve `AttendanceTokenService`.
- Backend `POST /events/:eventId/attendance-token` endpointi eklendi; kulüp admini veya `SYSTEM_ADMIN` katılım penceresi (etkinlik başlangıcından 30d önce - bitişinden 30d sonra) içindeki `PUBLISHED` etkinlik için HMAC-SHA256 imzalı 90 saniyelik QR katılım tokenı üretir (`QR_ATTENDANCE_SECRET`).
- Backend `POST /attendance/check-in` endpointi eklendi; `STUDENT` kullanıcısı `CheckInRequest` tokenı ile check-in yapar. Token imzası, süresi ve amacı stateles olarak doğrulanır, etkinlik ve kayıt durumu kontrol edilir, `Attendance` kaydı ve `EVENT_ATTENDANCE_RECORDED` audit kaydı atomik olarak oluşturulur.
- `CheckInRequest` contract arayüzü `@agu/contracts` paketine eklendi.
- `QR_ATTENDANCE_SECRET`, `QR_ATTENDANCE_TOKEN_TTL_SECONDS = 90`, katılım penceresi sabitleri (`30d/30d`) `@agu/config` paketine eklendi.
- Web `/club-dashboard/events/[eventId]/attendance` route'u ve `AttendanceQrLiveScreen` organizatör canlı QR ekranı bileşeni eklendi.
- Canlı QR ekranı yetkili kulüp admini veya `SYSTEM_ADMIN` yetkisini `/auth/me` üzerinden kontrol eder; yetkisiz kullanıcılara (`STUDENT`, `CLUB_MEMBER`, yalnızca `PRESS_EDITOR`) ve oturumsuz kullanıcılara kontrollü mesaj gösterir.
- `expiresAt` üzerinden dinamik kalan süre hesaplamasıyla otomatik yenileme (kalan süre <= 25s olduğunda veya süre dolduğunda) uygulanır.
- `document.visibilityState` takibiyle sekme gizlendiğinde yenilemeler duraklatılır, sekme tekrar açıldığında token süresi kontrol edilip gerekirse otomatik yenilenir.
- Ham token saklama/sızdırma önlemleri tam olarak uygulandı; token `localStorage`, `sessionStorage`, cookie veya URL query string'ine yazılmaz, DOM'da plain text olarak basılmaz.
- Web `/club-dashboard` üzerindeki `PUBLISHED` etkinlik kartlarına "Canlı QR Katılım Ekranı" bağlantısı eklendi.
- Web `/attendance/check-in` route'u eklendi; ana sayfaya (`/`) "QR ile Katılım" butonu eklendi.
- Öğrenci QR katılım ekranında `/auth/me` ile `STUDENT` rolü doğrulanır; oturumsuz veya yetkisiz kullanıcılara kontrollü mesaj gösterilir.
- Kamera yalnızca kullanıcı "Kamerayı Başlat" dediğinde açılır; unmount veya sayfa değişiminde `stopQrScannerSafely` ile kapatılır.
- QR payload `version=1`, `eventId`, `token` (opsiyonel `type="agu-attendance"`) yapısıyla doğrulanır; bozuk QR'lar API'ye gönderilmeden istemci tarafında engellenir.
- Check-in isteğinde `POST /attendance/check-in` body'sine yalnızca `{ "token": "..." }` yazılır; `400`, `401`, `403`, `404`, `409` yanıtları Türkçe mesajlara dönüştürülür.
- Başarılı check-in sonrası "Başka QR Tara" butonuyla tarama sıfırlanabilir; ham token DOM, URL veya storage'a yazılmaz.
- Backend `GET /events/:eventId/attendance-summary` endpointi eklendi; yetkili kulüp admini veya `SYSTEM_ADMIN` yetkisine açık, `STUDENT`, `CLUB_MEMBER` ve başka kulüp adminlerine `403` döndürür.
- `GET /events/:eventId/attendance-summary` response'unda `event`, `summary` (`registeredCount`, `attendanceCount`, `absentCount`, `capacityRemaining`, `registrationRate`, `attendanceRate`), `attendees` (`userId`, `displayName`, `email`, `registeredAt`, `checkedInAt`) ve `pagination` alanları döndürülür.
- `q` arama filtresi ve `page`, `pageSize` pagination parametreleri desteklendi.
- Web `/club-dashboard/events/[eventId]/attendance-summary` route'u ve `AttendanceSummaryScreen` eklendi; kulüp dashboard kartlarına ve organizatör QR canlı yayınına "Katılım Sonuçları" bağlantısı eklendi.
- `validateProductionEnv()` merkezi ortam değişkeni doğrulayıcısı eklendi; production ortamında DATABASE_URL, min 32 karakter AUTH_SESSION_SECRET, min 32 karakter varsayılan olmayan QR_ATTENDANCE_SECRET, wildcard olmayan WEB_ORIGIN ve ENABLE_DEV_AUTH!=true kurallarını zorunlu kılar.
- Dev-login production engeli ve startup validation ile dev auth'un production'da açılması kesin olarak engellendi.
- API ve Web yanıtlarına temel güvenlik başlıkları (X-Content-Type-Options, X-Frame-Options, X-XSS-Protection, Referrer-Policy, HSTS) eklendi.
- Hassas endpoint'ler (`/auth/dev-login`, `/attendance/check-in`, `/events/*/attendance-token`) için per-IP in-memory rate-limiter middleware uygulandı.
- `GET /health` (liveness) ve `GET /ready` (veritabanı canlılık kontrolü - SELECT 1) endpoint'leri ayrıldı; hazır olmama durumunda 503 HTTP koduyla kontrollü yanıt döndürür.
- AGÜ'ye Özel ve Dış Katılıma Açık Etkinlik Ayrımı ve `EXTERNAL_PARTICIPANT` rolü eklendi; Prisma `UserRole` enum'una `EXTERNAL_PARTICIPANT`, `Event` modeline `participationScope` (`AGU_ONLY`, `EXTERNAL_ALLOWED`) eklendi. Etkinlik oluşturma ve düzenleme formlarına Katılım Kapsamı seçeneği eklendi. Public etkinlik kartlarında ve detay başlığında "AGÜ Katılımcılarına Özel" / "Dış Katılıma Açık" rozetleri gösterildi. Kayıt ve QR check-in servislerinde katılım kapsamı kontrolü uygulandı; `EXTERNAL_PARTICIPANT` kullanıcılarının kulüp yönetimi, etkinlik oluşturma/düzenleme/onaylama, QR üretimi veya raporlama yapması engellendi. Katılım özeti tablosuna "Katılımcı Türü" (AGÜ vs Dış) eklendi. Unit ve integration testleri yazıldı.
- E-posta tek kullanımlık kod (OTP) ile giriş sisteminin backend temeli eklendi; Prisma `EmailLoginChallenge` modeli (`20260724110000_add_email_login_challenge`), `POST /auth/email/request-code` ve `POST /auth/email/verify-code` endpointleri, HMAC-SHA256 OTP hashleme, `EmailDeliveryService` soyutlaması (Console & Smtp sağlayıcıları), 10 dakika TTL, 60s cooldown, max 5 hatalı deneme kilidi, `@agu.edu.tr` tam domain kontrolü ile otomatik `STUDENT` veya `EXTERNAL_PARTICIPANT` rol ataması, `agu_session` cookie entegrasyonu, rate-limiting, log sanitization ve kapsamlı unit/integration testleri tamamlandı.
- E-posta tek kullanımlık kod (OTP) ile giriş web arayüzü ve uygulama içi auth yönlendirmeleri geliştirildi; `/login` route'u (`LoginPage`, `LoginScreen`, `EmailLoginForm`), `auth-utils.ts` (güvenli `returnTo` doğrulaması, e-posta/OTP format temizliği, güvenli API hata eşlemesi), 3 adımlı form akışı (E-posta -> 6 Haneli Kod -> İlk Kullanıcı Ad Soyad), 60s resend cooldown sayacı, unmount timer temizliği, `SiteHeader` bileşeni (session bazlı Giriş Yap / Çıkış Yap paneli), etkinlik detay kayıt paneli (`returnTo=/events/<eventId>`) ve QR check-in ekranı (`returnTo=/attendance/check-in`) entegrasyonları, OTP gizlilik ve güvenlik korumaları (storage/URL/console sızdırmazlığı) ve kapsamlı web testleri yazıldı.
- Google Cloud Run dağıtımı için altyapı eklendi: `apps/api/src/config/port-resolver.ts` ile API artık `process.env.PORT` -> `process.env.API_PORT` -> `DEFAULT_API_PORT` sırasıyla portu çözer ve `0.0.0.0` üzerinde dinler (`main.ts`). `apps/api/Dockerfile` (hedefler: `api`, `migrate`) ve `apps/web/Dockerfile` (Next.js `output: "standalone"`, bkz. `next.config.ts`) eklendi; ikisi de Node 24 + pnpm 11.16.0, `turbo prune` ile pruned monorepo, non-root kullanıcı kullanır. `cloudbuild.api.yaml` / `cloudbuild.web.yaml` Artifact Registry'ye commit SHA + latest tagleriyle push eder. `docs/pilot/DEPLOYMENT_GCP.md` Cloud Run servis/job deploy komutlarını, secret isimlerini (değersiz) ve SMTP açılış adımını belgeler. `apps/api/src/deploy/docker-deploy.spec.ts` Dockerfile/Cloud Build içeriğini Docker'sız doğrular; `scripts/docker/*.sh` Docker gerektiren container smoke testleri içerir.
...
- API smoke: `curl http://localhost:3001/health` ve `curl http://localhost:3001/ready`
- Web smoke: `curl http://localhost:3000/` API acik ve kapaliyken

## Bilinen Eksikler

- Gercek AGU SSO entegrasyonu yok; pilot ortamında SSO henüz olmadığı için PILOT_READINESS belgesinde kritik blocker olarak işaretlendi.
- Bildirim adapterleri placeholder mimari sinir olarak duruyor.
- Çoklu-instance production dağıtımları için Redis tabanlı rate-limiter gerekebilir (şu anki pilot in-memory).
...
- Unit tests: gecti; API 218 test, contracts 2 test, web 140 test.
- Integration tests: gecti; API 153 integration test across 6 test suites.
